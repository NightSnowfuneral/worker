...
class CustomFromBar extends Component{
		...
		switch(item.type) {
			...
			case 'DataPicker':              //时间
				return( 
						getFieldDecorator(item.dataIndex,{
							initialValue:item.value,
							rules:[{required:item.require , message:'时间' + item.title}]
						})(
							 <DatePicker
				              	disabled={item.disabled} 
	...
	handleSubmit(e){             //提交表单的概念
		...
		this.props.form.validateFieldsAndScroll( async (err, value) => {
			...
			if (!err) {
				...
				dataValue.time = value.LogTime.format('YYYY-MM-DD HH:mm:ss')
			...
			if (this.props.types == 40) {    //角色
				...
				if (res.code == 200) {
				 	...
				 	this.setState({submitBtnLoading:false,listData:[]})
				 	this.props.closeModal()
				 	this.handleReset()
				 	const {actions} = this.props
					actions.RulePeole()
	...
	handleHolds(e){   //暂存的概念
		...
		this.props.form.validateFieldsAndScroll( async (err, value) => {
			...
			if(CheckCharacter(dataValue.description)){
				message.error("有特殊字符不能暂存")
				return
			}
